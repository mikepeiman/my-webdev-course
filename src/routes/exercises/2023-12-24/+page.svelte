<script>
	...
	$: unitHeight =
		settings.radius > (global_height - settings.radius * 2) / settings.numHexesInCol
			...
			: settings.radius;
	$: calculatedRadius = Math.max(unitWidth, unitHeight);
	$: console.log(`🚀 ~ file: +page.svelte:27 ~ calculatedRadius:`, calculatedRadius)
	$: console.log(`🚀 ~ file: +page.svelte:abc ~ unitWidth: ${unitWidth} unitHeight: ${unitHeight}`);
	// $: hex = calculateHexagonPoints(settings.radius, 0, 0, true);
	$: hexes = [];
	$: console.log(`🚀 ~ file: +page.svelte:31 ~ hexes:`, hexes)
